...
let arrGenres = [];
const genres = films.map((el) => arrGenres.push(...el.genre));
let arrGenresUnic = arrGenres.reduce((result, item) => {
  return result.includes(item) ? result : [...result, item];
}, []);

// 2. Собрать в массив всех актеров всех фильмов (без повторения)
let arrActors = [];
const actors = films.map((el) => arrActors.push(...el.actors));
let arrActorsUnic = arrActors.reduce((acc, el) => {
  return acc.includes(el) ? acc : [...acc, el];
}, []);
console.log(arrActorsUnic);

// 3. Отсортировать фильмы по рейтингу по убыванию
const rating = films.sort((a, b) => (b.imdbRating > a.imdbRating ? 1 : -1));
console.log(rating);

// 4. Создать новый массив, где объекты фильмов будут состоять из следующих полей: id, title, released, plot
const film = films.map((el) => ({
  id: el.id,
  title: el.title,
...
let numb = 2001;
const data = (arr, number) => arr.filter(({ year }) => year == number);
console.log(data(films, numb));
...
let str = "Black";
const filmTitle = (arr, string) =>
  arr.filter(({ title }) => title.includes(string));
console.log(filmTitle(films, str));

// 7. Создать функцию, которая бы принимала массив фильмов и строку. А результатом этой функции должен быть oтфильтрованный массив, с фильмами где строка входит в название фильма или в его сюжет.
let titleAndPlot = "learn more";
const filmTitleAndPlot = (arr, string) =>
  arr.filter((el) => el.title.includes(string) || el.plot.includes(string));
console.log(filmTitleAndPlot(films, titleAndPlot));
...
let string1 = "year";
let string2 = 2011;
const findFilm = (arr, key, value) => arr.filter((el) => el[key] === value);
console.log(findFilm(films, string1, string2));
